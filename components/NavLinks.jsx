...
const externalLinks = [
  {
    name: 'Instagram',
    link: 'https://www.instagram.com/sketchie32',
    icon: 'tbd',
  },
  {
    name: 'LinkedIn',
    link: 'https://www.linkedin.com/in/elliely32/',
    icon: 'tbd',
  },
  {
    name: 'GitHub',
    link: 'https://github.com/elliely32',
    icon: 'tbd',
  },
];
...
const Home = ({ containerStyles }) => {
  ...
  const externalLinkImagePathHash = {
    LinkedIn:
  ...
  const externalLinkImage = (linkName) => {
    return (
      <span className="h-5 w-5 text-primary">
        <svg
          ...
          viewBox="0 0 488 512"
        >
          <path d={externalLinkImagePathHash[linkName]} />
        </svg>
      </span>
    );
  };
  ...
  return (
    <>
      <ul className="pt-35 2xl:pt-0 flex flex-col justify-center items-center 2xl:items-start gap-6 h-full">
        {links.map((link, idx) => {
          return (
            <Link
              href={link.path}
      ...
      <div className="flex flex-row gap-3 pt-35 justify-center">
          ...
          return (
            ...
            >
              <div className="w-10 h-10 rounded-full flex justify-center items-center bg-white">
                {externalLinkImage(externalLink.name)}
              </div>
            </a>
          );
        })}
